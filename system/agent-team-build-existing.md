...
1. **Grep Local Codebase FIRST (NON-NEGOTIABLE)**: Before writing ANY code, grep THIS project to study existing patterns.
2. **Grep MCP (NON-NEGOTIABLE)**: AFTER grepping local, use `grep_query` to search GitHub for battle-tested code.
3. **LSP After Every Edit (NON-NEGOTIABLE)**: `getDiagnostics` after EVERY edit. `goToDefinition` before modifying. `findReferences` before renaming.
4. **Plan Before Execute (NON-NEGOTIABLE)**: Outline plan BEFORE non-trivial changes.
5. **Learn From Mistakes (NON-NEGOTIABLE)**: Read `LEARNINGS.md` at start. Write concise learnings at end.
6. **Task Management (NON-NEGOTIABLE)**: ALL work tracked via TaskUpdate. `in_progress` when starting, `completed` only after tests pass.
...
10. **Test after change**: Run `{DETECTED_TEST_RUNNER_COMMAND}` after any code change
11. **Lint after change**: Run `{DETECTED_LINTER_COMMAND}` after any code change
